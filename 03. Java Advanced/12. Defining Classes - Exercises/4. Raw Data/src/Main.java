...
public class Main {
    public static void main(String[] args) {
        ...
        int carsCount = Integer.parseInt(keyboard.nextLine());
        ...
        Predicate<Car> filterCars = getCarFilter(keyboard.nextLine());
    ...
    private static Predicate<Car> getCarFilter(String filterType) {
        if (filterType.equals("fragile")) {
            return car -> car.getCargo().getType().equals(filterType) &&
                    Arrays.stream(car.getTires()).anyMatch(tire -> tire.getPressure() < 1);
        } else {
            return car -> car.getCargo().getType().equals(filterType) &&
