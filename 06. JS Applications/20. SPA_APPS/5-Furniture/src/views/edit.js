...
let editTemplate = (itemPromise) => html`
...
let formTemplate = (item, onSubmit, errorMsg, errors) => html`
...
export function editPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        let data = formData.reduce((acc, [key, value]) => Object.assign(acc, { [key]: value.trim() }), {});

        let missingField = formData.filter(([k, v]) => k != "material" && v.trim() == "");

        try {
            if (missingField.length > 0) {
                let errors = missingField.reduce((a, [k]) => Object.assign(a, { [k]: true }), {});
                console.log(errors);
                throw {
            ...
            let result = await editItem(ctx.params.id, data);
            event.target.reset();
